...
describe("PO Parser", () => {
	...
	describe("parsing errors", () => {
		...
		it("should throw (stream with unescaped quote)", (done) => {
			const poStream = fs.createReadStream(
				path.join(__dirname, "fixtures/error-unescaped-quote.po"),
				{
					highWaterMark: 1, // ensure that any utf-8 sequences will be broken when streaming
				},
			);
		...
		describe("when validation is disabled", () => {
			...
			it("should throw (unescaped quote)", async () => {
				...
				expect(
					gettextParser.po.parse.bind(gettextParser.po, po, options),
				).to.throw(invalidKeyError);
			});

			it("should throw (double-escaped quote)", async () => {
				const po = await readFile(
					path.join(__dirname, "fixtures/error-double-escaped-quote.po"),
				);
			...
			it("should not throw (an entry has too few plural forms)", async () => {
				const po = await readFile(
					path.join(__dirname, "fixtures/validate-too-few-plural-forms.po"),
				);
			...
			it("should not throw (an entry has too many plural forms)", async () => {
				const po = await readFile(
					path.join(__dirname, "fixtures/validate-too-many-plural-forms.po"),
				);
			...
			it('should not throw (an entry misses "msgid_plural")', async () => {
				const po = await readFile(
					path.join(__dirname, "fixtures/validate-missing-msgid-plural.po"),
				);
			...
			it('should not throw (an entry misses single "msgstr")', async () => {
				const po = await readFile(
					path.join(__dirname, "fixtures/validate-missing-msgstr.po"),
				);
			...
			it("should not throw (duplicate entries found in the same context)", async () => {
				const po = await readFile(
					path.join(
						__dirname,
						"fixtures/validate-context-duplicate-entries.po",
					),
				);
			...
			it('should not throw (an entry with multiple "msgid_plural")', async () => {
				const po = await readFile(
					path.join(__dirname, "fixtures/validate-redundant-msgid-plural.po"),
				);
